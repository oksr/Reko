...
function EditorContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      platform.invoke<ProjectState>("load_project", { projectId }),
      platform.invoke<WallpaperInfo[]>("list_wallpapers").catch(() => [] as WallpaperInfo[]),
    ])
      .then(async ([p, wallpapers]) => {
        ...
        if (bg.type === "wallpaper" && bg.wallpaperId && !bg.imageUrl) {
          try {
            const resolvedPath = await platform.invoke<string>("resolve_wallpaper_path", {
