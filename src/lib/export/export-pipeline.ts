...
export class ExportPipeline {
  ...
  async run(
    ...
    try {
      // 1. Load screen video
      const videoUrl = assetUrl(project.tracks.screen)
      ...
      if (project.tracks.camera) {
        try {
          this.cameraVideo = await loadVideo(assetUrl(project.tracks.camera))
        } catch (e) {
      ...
      if (project.tracks.mouse_events) {
        mouseEvents = await loadMouseEvents(assetUrl(project.tracks.mouse_events))
      }
